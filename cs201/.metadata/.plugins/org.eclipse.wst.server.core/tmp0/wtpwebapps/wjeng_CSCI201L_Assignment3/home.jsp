...
<html>
	<head>
		...
		<script>
			//Make all staff with matching name to have highlighted background
			function searchStaff(){
				resetSearch();
				var term = document.searchForm.searchTerm.value;
				term = term.toLowerCase();
				var xhttp = new XMLHttpRequest();
				xhttp.open("GET", "staffSearch.jsp?term="+term, false);
				xhttp.send();
			...
			//No input
			//Reset the backgroundcolor of all class
			function resetSearch(){
				...
				for(var i = 0; i < classNames.length; i++){
					...
					for(var k = 0; k < classElements.length; k++){
						if(classElements[k].style.backgroundColor === "red"){
				...
				<tr>
					...
					<td class = "nav" width="180" valign="top" align="right">
						<div id="sidebar">
							...
							<div class="navCont"><font size="+1"><%=myDepartment.getPrefix()+myCourse.getNumber()%> Home</font><br /><br /></div>
					...
					<td class="body" align="baseline" width="615">
						...
						<p>
				          ...
				          <button class="buttons search" type="button" onclick="searchStaff();">Search Staff</button>
				          <button class="buttons reset" type="reset" onclick="resetSearch();">Clear Search</button>
				          </form>
						  ...
						  <table border="1">
								<tr>
              					...
              					<tr>
              						...
              						<td>
              							<div class = "<%= professor.getName().getFname().toLowerCase() + " " + professor.getName().getLname().toLowerCase() %>">
              								<img class="pic" width="100" height="100" src=<%=professor.getImage()%> />
              							</div>
              						</td>
              						<td>
              							<div class = "<%= professor.getName().getFname().toLowerCase() + " " + professor.getName().getLname().toLowerCase() %>">
              								<font size="-1"><%=professor.getName().getFname() + " " + professor.getName().getLname()%></font>
              							</div>
              						</td>
									...
									for(int i = 0; i < lectures.size(); i++){
										...
										<%if(assistant !=null){
											if(assistant.getImage() == null){
												%><td align="center"><font size="-1">
													<div class = "<%= assistant.getName().getFname().toLowerCase() + " " + assistant.getName().getLname().toLowerCase()%>" >
														<img class="pic" src=<%= mySchool.getImage() %> /><br /><a href=<%="mailto:" + assistant.getEmail()%>><%=assistant.getName().getFname() + " "+assistant.getName().getLname() %></a></font>
													</div>
												</td> 
												<%
											}
												...
												%><td align="center">
													<div class = "<%= assistant.getName().getFname().toLowerCase() + " " + assistant.getName().getLname().toLowerCase()%>" >
														<font size="-1"><img class="pic" src=<%= assistant.getImage() %> /><br /><a href=<%="mailto:" + assistant.getEmail()%>><%=assistant.getName().getFname() + " "+assistant.getName().getLname() %></a></font>
													</div>
												</td> 
												<%
											}
											...
											<%
												...
												for(int j = 0; j < labStaff.size(); j++){
													if(labStaff.get(j).getImage() == null){
														%>
														<td align="center">
															<div class = "<%= labStaff.get(j).getName().getFname().toLowerCase() + " " + labStaff.get(j).getName().getLname().toLowerCase() %>">
																<font size="-1">													
																	<img class="pic" src=<%= mySchool.getImage() %>><br /><a href=<%= "mailto:"+labStaff.get(j).getEmail() %>><%= labStaff.get(j).getName().getFname() +" " +labStaff.get(j).getName().getLname()  %></a>														
																</font>		
															</div>												
														</td>
														<%
														...
														<td align="center">
															<div class = "<%= labStaff.get(j).getName().getFname().toLowerCase() + " " + labStaff.get(j).getName().getLname().toLowerCase() %>">
																<font size="-1">
																	<img class="pic" src=<%= labStaff.get(j).getImage() %>><br /><a href=<%= "mailto:"+labStaff.get(j).getEmail() %>><%=labStaff.get(j).getName().getFname() +" " +labStaff.get(j).getName().getLname() %></a>
																</font>
															</div>
														</td>
														<%
					            	...
					            	for(int i = 0; i < 6; i++){
					            		...
					            		<% for(int j = 0; j < 5; j++){            			
					            			%><td><% if(officeHours[j][i]!=null){
					            				%>
					            					<div class="<%=officeHours[j][i].getName().getFname().toLowerCase() +" "+officeHours[j][i].getName().getLname().toLowerCase() %>">
					            						<img class="pic" src=<%= officeHours[j][i].getImage()%> /><br />
					            						<font size="-1"><a href=<%= "mailto:"+ officeHours[j][i].getEmail() %>><%= officeHours[j][i].getName().getFname() + " " + officeHours[j][i].getName().getLname() %></a><br />&nbsp;
					            						</font>
					            					</div> </td><%
					            			}
					            			//Case if no img avialble
					            			else{
					            				%><img class="pic" src=<%= mySchool.getImage()%> /><br />
					            				<font size="-1">No Staff Available<br />&nbsp;</font></td><%
					            			}
					            		}
					            	}
					            %>
